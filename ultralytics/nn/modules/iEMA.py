...
class LayerNorm2d(nn.Module):
    ...
    def get_norm(norm_layer='in_1d'):
        eps = 1e-6
        norm_dict = {
                'none': nn.Identity,'in_1d': partial(nn.InstanceNorm1d, eps=eps),
...
class ConvNormAct(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, kernel_size, stride=1, dilation=1, groups=1, bias=False,
        ...
        self.norm = LayerNorm2d.get_norm(norm_layer)(dim_out)
        self.act = LayerNorm2d.get_act(act_layer)(inplace=inplace)
...
class iRMB_EMA(nn.Module):
    ...
    def __init__(self, dim_in, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
        ...
        super().__init__()
        dim_out = dim_in
        self.norm = LayerNorm2d.get_norm(norm_layer)(dim_in) if norm_in else nn.Identity()
        ...
        self.se = SqueezeExcite(dim_mid, rd_ratio=se_ratio,
                                act_layer=LayerNorm2d.get_act(act_layer)) if se_ratio > 0.0 else nn.Identity()
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, iRMB_EMA.autopad(k, p, d), groups=g, dilation=d, bias=False)
