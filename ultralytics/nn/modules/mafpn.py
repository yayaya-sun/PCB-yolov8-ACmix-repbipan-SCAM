    ...
    def __init__(self,
        ...
        if use_depthwise:
            self.conv2 = ReparamLargeKernelConv(in_channels=mid_channel, out_channels=out_channels,
                                                kernel_size=kersize, stride=1, groups=mid_channel,
...
class Head_out(nn.Module):
    def __init__(self, in_channels, out_channels, reg_max=16, num_classes=3, num_anchors=1):
        ...
        # reg_conv0
        self.reg_conv = Conv(in_channels=out_channels, out_channels=4 * (reg_max + num_anchors), kernel_size=3,
                             stride=1)
        # cls_pred0
        self.cls_pred = nn.Conv2d(in_channels=num_classes * num_anchors, out_channels=num_classes * num_anchors,
                                  kernel_size=1)
        # reg_pred0
        self.reg_pred = nn.Conv2d(in_channels=4 * (reg_max + num_anchors), out_channels=4 * (reg_max + num_anchors),
                                  kernel_size=1)
        self.prior_prob = 1e-2
        self.initialize_biases()
...
class RepELANMS2(nn.Module):
    def __init__(self, in_channels, out_channels, depth=1, shortcut=True, expansion=0.5, kersize=5, depth_expansion=1,
        ...
        self.conv1 = Conv(in_channels, c1, 1, 1)
        self.m1 = DepthBottleneckUni(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize, use_depthwise)
        self.m11 = DepthBottleneckUni(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize,
                                      use_depthwise)
        self.m2 = DepthBottleneckUni(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize, use_depthwise)
        self.m22 = DepthBottleneckUni(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize,
                                      use_depthwise)
...
class CSPDepthResELAN(nn.Module):
    def __init__(self, in_channels, out_channels, depth=1, shortcut=True, expansion=0.5, kersize=5, depth_expansion=1,
        ...
        self.m = nn.ModuleList(
            DepthBottleneck(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize, use_depthwise) for _ in
            range(depth))
...
class CSPSDepthResELAN(nn.Module):
    def __init__(self, in_channels, out_channels, depth=1, shortcut=True, expansion=0.5, kersize=5, depth_expansion=1,
        ...
        self.conv1 = Conv(in_channels, c1, 1, 1)
        self.m = nn.ModuleList(
            SDepthBottleneck(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize, use_depthwise) for _
            in range(depth))
        self.conv2 = Conv(c_ * (depth + 2), out_channels, 1, 1)
...
class SDepthMP(nn.Module):
    def __init__(self, in_channels, out_channels, depth=1, shortcut=True, kersize=5, small_kersize=3, depth_expansion=2,
                 expension_w=3, use_depthwise=True):
        ...
        self.m1 = nn.ModuleList(
            SDepthBottleneck(self.mid_channel, self.mid_channel, shortcut, kersize, depth_expansion, small_kersize,
                             use_depthwise) for _ in range(depth))
        self.m2 = nn.ModuleList(
            Bottleneck2(self.mid_channel, self.mid_channel, shortcut, kersize, 1, small_kersize, use_depthwise) for _ in
            range(depth))
        self.shortcut = shortcut
