...
import glob
'''
该文件用于将xml标注文件格式转换为txt格式
'''
# 1.
# 自己创建文件夹,例如：Annotations labels  也可以修改别的
image_set = "Annotations_DA"  # 需要转换的文件夹名称（文件夹内放xml标签文件）
imageset2 = "labels_DA"  # 保存txt的文件夹
# 2.
# 换成你的类别 当前的顺序，就txt 0,1,2,3 四个类别
#classes = ['J','K','C','D','T','R','F','H','notok']  # 标注时的标签 注意顺序一定不要错。
#classes = ['R', 'C', 'J', 'D', 'Y', 'notok']  # 标注时的标签 注意顺序一定不要错。
classes = ['missing_hole', 'mouse_bite', 'open_circuit', 'short', 'spur', 'spurious_copper']  # 标注时的标签 注意顺序一定不要错。
