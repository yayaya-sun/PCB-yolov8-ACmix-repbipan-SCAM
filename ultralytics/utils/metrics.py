...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    # 如果 names 是字典，提取其值（标签名列表）
    if isinstance(names, dict):
        names = list(names.values())

    # 强制映射标签名
    try:
        names = [name_mapping.get(name, name) for name in names]  # 映射标签
    except Exception as e:
        print(f"Error mapping names: {e}, names={names}")

    print(f"Mapped names: {names}")  # 调试信息，确保映射后 names 正确
